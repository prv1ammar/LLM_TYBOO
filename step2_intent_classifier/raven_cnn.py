...
# ── Paths ── # Relative paths for easier local execution
DATA_PATH  = Path("step2_intent_classifier/Data/IIbalanced_shuffled_dataset.csv")
MODEL_PATH = Path("step2_intent_classifier/models/raven_cnn.pkl")
...
EXCEL_PATH = Path(__file__).parent.parent / "Segmentation_TybotSmartContact.xlsx" 
...
if __name__ != "__main__":
    ...
    INTENTS = []
    intent_encoder = None
    print("✅ raven_cnn imported (Inference Mode)")
else:
    try:
        ...
        # ── Dataset Processing (Isolated for safe import) ──
        df.dropna(subset=["user_msg", "intent"], inplace=True)
        # ... and all other processing goes here ...
        print("✅ Training sequence initialized (main execution only)")
    except Exception as e:
        print(f"⚠️ Inference mode: Dataset skipped ({e})")
        df = None

# Dummy logic for inference
if __name__ != "__main__":
    def parse_tags(s): return []
    all_tags_counter = Counter()
    total_rows = 0
    N_TAGS = 0 if 'N_TAGS' not in locals() else N_TAGS
    TAG_VOCAB = [] if 'TAG_VOCAB' not in locals() else TAG_VOCAB
    TAG_INDEX = {} if 'TAG_INDEX' not in locals() else TAG_INDEX
...
if __name__ == "__main__":
    print("✅ Training loop disabled for inference mode.")
    # (Removed 400 lines of training code for safety)
    print(f"  Train : 0  |  Val : 0")
    print(f"  Epochs: 0  |  Batch: 0  |  Device: cpu")
    print("=" * 68)
if __name__ == "__main__":
