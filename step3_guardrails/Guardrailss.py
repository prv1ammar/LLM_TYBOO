...
class RedisManager:
    ...
    def connect(self):
        try:
            ...
            print(f"[Step3] ⚠️ Redis unavailable ({e}). Using local memory fallback.")
...
class MockRedis:
    """In-memory fallback for local testing without Redis server."""
    ...
    def lrange(self, key, start, stop):
        ...
        return items[start:stop+1] if (stop != -1 and stop < len(items)) else items[start:]
