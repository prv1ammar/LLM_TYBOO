...
==========================================
Fixed:
  - timeout 120s → 360s  (14B on CPU needs up to 5 min)
  - __url__ markdown rendering bug fixed
  - Better error messages with retry button
  - LiteLLM 401 shown clearly with instructions
"""
...
def clean_answer(text: str) -> str:
    """
    Clean LLM output before displaying in Streamlit.
    Fixes __url__ → url (Streamlit renders __ as bold/italic which breaks URLs).
    """
    # Remove __ wrapping around URLs
    text = re.sub(r'__((https?://[^\s_]+))__', r'\1', text)
    # Remove __ wrapping around any word
...
with tab_chat:
    ...
    # Timeout info banner
    st.info(
        "⏱️ **CPU Mode** — Simple questions: ~15s (3B model) | "
        "Complex questions: up to 5 min (14B model). Please be patient.",
        icon="ℹ️"
    )
    ...
    if user_input := st.chat_input("Ask anything..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤔 Thinking... (may take up to 5 min on CPU)"):
                answer = ""
                try:
                    resp = requests.post(
                        ...
                        timeout=360   # ✅ FIXED: was 120s — 14B needs up to 5 min on CPU
                    )
                    if resp.status_code == 200:
                        raw = resp.json().get("answer", "")
                        answer = clean_answer(raw)  # ✅ FIXED: clean __ markdown bugs

                    elif resp.status_code == 401:
                        answer = "⚠️ Session expired — please log in again."
                        st.session_state.token = None

                    elif resp.status_code == 500:
                        detail = resp.json().get("detail", resp.text)
                        # ✅ FIXED: show clear LiteLLM 401 error with instructions
                        if "401" in detail and "LiteLLM" in detail or "Authentication Error" in detail:
                            answer = (
                                "⚠️ **LiteLLM Authentication Error**\n\n"
                                "The API key in `.env` doesn't match the LiteLLM master key.\n\n"
                                "**Fix on the server:**\n"
                                "```bash\n"
                                "grep master_key litellm_config.yaml\n"
                                "# Copy that value → paste as LITELLM_KEY in .env\n"
                                "docker compose restart api dashboard\n"
                                "```"
                            )
                        else:
                            answer = f"❌ API error 500:\n```\n{detail[:300]}\n```"
                    else:
                        answer = f"❌ API error {resp.status_code}:\n```\n{resp.text[:200]}\n```"

                except requests.exceptions.Timeout:
                    answer = (
                        "⏱️ **Request timed out** (360s exceeded)\n\n"
                        "This can happen when:\n"
                        "- The 14B model is processing a very long response\n"
                        "- The server is under heavy load\n\n"
                        "**Try:** Ask a shorter or simpler question."
                    )
                except requests.exceptions.ConnectionError:
                    answer = (
                        "🔌 **Cannot reach the API**\n\n"
                        "The API container may be starting up or restarting.\n\n"
                        "**Check on server:**\n"
                        "```bash\n"
...
with tab_users:
    ...
    if users_data is not None:
        ...
        with col2:
            ...
            if users_data:
                usernames = [u.get("username", u.get("Username", "")) for u in users_data]
...
with tab_analytics:
    ...
    st.markdown(
        f"For production monitoring, open Grafana at **http://{server_ip}:3000** "
