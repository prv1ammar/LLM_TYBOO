...
from step3_guardrails.Guardrailss import GuardrailPipeline, ScanResult
...
class Config:
    ...
    REDIS_PORT     = 6379
    LOG_DIR        = "logs"
    INTENT_MODEL   = "step2_intent_classifier/models/raven_cnn.pkl"
    LLM_MODEL      = "Qwen/Qwen2.5-1.5B-Instruct"   # main chat model (matches your adapter)
    ADAPTER_PATH   = "step6_finetuning/model"       # path to your LoRA adapter
...
5. NEVER expose personal data — use masked tokens like [RIB_MASQUÉ] if needed"""
    ...
    def _load(self):
        ...
        print(f"[LLM] Download/Cache check for {self.model_path}...")
        self._tokenizer = AutoTokenizer.from_pretrained(self.model_path)
        
        print(f"[LLM] Loading base model weights (approx 6GB RAM)...")
        ...
        print("[LLM] Base model loaded!")
        
        if self.adapter_path and os.path.exists(self.adapter_path):
            try:
                print(f"[LLM] Applying LoRA adapter (Step 6)...")
                from peft import PeftModel
                self._model = PeftModel.from_pretrained(self._model, self.adapter_path)
                print("[LLM] LoRA adapter applied successfully!")
            except ImportError:
                print("[LLM] ⚠️ skip adapter loading: pip install peft required")
            except Exception as e:
                print(f"[LLM] ⚠️ failed to load adapter: {e}")
...
_classifier: Optional[IntentClassifierWrapper] = None
